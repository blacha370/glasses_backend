...
class DeleteMessageThreadTestCase(TestCase):
    ...
    def test_post_method_delete_not_existing_message(self):
        self.user.groups.add(self.groups['administracja'], self.groups['4dich'])
        self.client.force_login(self.user)
        response = self.client.post('/message/delete/2', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
    ...
    def test_put_method_without_authentication(self):
        response = self.client.put('/message/delete/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/?next=/message/delete/1')
        self.assertEqual(response.redirect_chain[0][1], 302)
    ...
    def test_put_method_with_authentication_with_wrong_administracja_group(self):
        self.user.groups.add(self.groups['administracja'], self.groups['besart'])
        self.client.force_login(self.user)
        response = self.client.put('/message/delete/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
    ...
    def test_put_method_with_authentication_as_proper_group(self):
        self.user.groups.add(self.groups['administracja'], self.groups['4dich'])
        self.client.force_login(self.user)
        response = self.client.put('/message/delete/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
    ...
    def test_put_method_delete_not_existing_message(self):
        self.user.groups.add(self.groups['administracja'], self.groups['4dich'])
        self.client.force_login(self.user)
        response = self.client.put('/message/delete/2', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
    ...
    def test_delete_method_with_authentication_with_wrong_administracja_group(self):
        self.user.groups.add(self.groups['administracja'], self.groups['besart'])
        self.client.force_login(self.user)
        response = self.client.delete('/message/delete/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
    ...
    def test_delete_method_with_authentication_as_proper_group(self):
        self.user.groups.add(self.groups['administracja'], self.groups['4dich'])
        self.client.force_login(self.user)
        response = self.client.delete('/message/delete/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
    ...
    def test_delete_method_delete_not_existing_message(self):
        self.user.groups.add(self.groups['administracja'], self.groups['4dich'])
        self.client.force_login(self.user)
        response = self.client.delete('/message/delete/2', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
