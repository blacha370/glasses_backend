...
class ArchiveThreadTestCase(TestCase):
    ...
    def test_with_authentication_as_proper_group(self):
        ...
        self.client.force_login(self.user)
        response = self.client.get('/message/thread/1/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/message/thread/1/1')
        self.assertEqual(len(response.redirect_chain), 0)
        self.assertEqual(response.templates[0].name, 'page/message.html')
        self.assertEqual(response.templates[1].name, 'page/base.html')
        ...
        self.client.force_login(self.user)
        response = self.client.get('/message/thread/1/2', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/message/thread/1/2')
        self.assertEqual(len(response.redirect_chain), 0)
        self.assertEqual(response.templates[0].name, 'page/message.html')
        self.assertEqual(response.templates[1].name, 'page/base.html')
        self.assertEqual(response.context[1].dicts[3]['notification'], 0)
    ...
    def test_post_method(self):
        self.user.groups.add(self.groups['druk'])
        self.client.force_login(self.user)
        response = self.client.post('/message/thread/1/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
    ...
    def test_put_method(self):
        self.user.groups.add(self.groups['druk'])
        self.client.force_login(self.user)
        response = self.client.put('/message/thread/1/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
    ...
    def test_delete_method(self):
        self.user.groups.add(self.groups['druk'])
        self.client.force_login(self.user)
        response = self.client.delete('/message/thread/1/1', follow=True)
        self.assertEqual(response.request['PATH_INFO'], '/inbox/')
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][0], '/inbox/')
        self.assertEqual(response.redirect_chain[0][1], 302)
